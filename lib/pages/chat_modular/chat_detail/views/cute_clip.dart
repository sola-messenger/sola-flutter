...
class CuteClip extends StatefulWidget {
  final Event event;

  const CuteClip(this.event, {Key? key}) : super(key: key);

  @override
  State<CuteClip> createState() => _CuteClipState();
}

class _CuteClipState extends State<CuteClip> {
  static final List<OverlayEntry> overlays = [];

  @override
  void initState() {
    if (AppConstants.autoplayImages && overlays.isEmpty) {
      addOverlay();
    }
    super.initState();
  }
